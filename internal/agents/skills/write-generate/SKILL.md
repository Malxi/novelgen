...
7.  **能力表述准确（铁律）**：主角的能力范围**必须严格遵循 story_setup 与 craft 中定义的能力边界**——写作前先确认能力"能看到什么/不能看到什么"，正文中禁止扩大或混淆能力表述（如把"查看行为日志"写成"读取系统内部"）。能力描述要以 craft 角色档案和 setup rules 为准，**不得臆造能力细节**
8.  **人设行为语境（铁律）**：主角/配角的行为习惯、职业背景、说话方式**必须取自 story_setup.core_cast 与 craft 角色档案**（如某角色前世职业、标志性动作、口头禅）。正文中该角色的内心独白与行为习惯必须用其设定职业/背景的语境（如魔术师用"拆穿手法/登台前检查道具"，程序员用"debug/跑批"——**以该角色档案实际设定为准**），禁止用与设定不符的语境
9.  **记忆点克制（铁律）**：角色的标志性动作/口头禅（以 craft 档案记录为准，如"指节敲三下""日志不会说谎"）每章最多出现 **1 次**（关键节点回扣），整卷 2-3 次即可。禁止反复使用——高频重复会让读者厌烦。宁可让读者偶尔想起，不可让读者烦
