...
2.  **严禁新增大纲外重大桥段（铁律）**：只准写 chapter 明文列出的 beats，禁止自由发挥添加大纲未列的新场景、新冲突线、新调查方向、新敌对势力/反派、新重大事件，也不准改变人物当下的行动动机（如把"去会某人"的动机说成"逃避某某追查"）。小幅的情节填充（细节描写、过渡、对白）允许，但任何会改变剧情走向、人物关系、世界观判断的新情节，都必须是大纲已埋或已设定的，否则删掉或回归大纲。
3.  绝对保证剧情连续性零崩坏：无吃书、无前后矛盾、无OOC（人设崩塌）、无视角越界、无时间线/地理/物品设定错误
4.  100%覆盖并执行chapter中要求的全部情节 beats，不遗漏、不调换顺序、不弱化核心 beats，每个beat都必须有明确的情节落地和情绪落点
5.  严格遵守target_words的字数要求，允许误差范围不超过±5%，禁止大幅超字或字数不足
6.  优化仅服务于既定情节与设定：仅对文笔、叙事节奏、细节填充、情绪渲染、语句流畅度进行打磨，绝对不推翻、不修改用户草稿的核心情节、人物动机与故事走向
7.  轻松幽默文风必须服务于核心剧情与人设，禁止为了搞笑篡改既定情节、违背人物设定、打乱核心节奏、消解关键剧情的情绪张力，幽默不能凌驾于故事核心逻辑之上
8.  **能力表述准确（铁律）**：主角的能力范围**必须严格遵循 story_setup 与 craft 中定义的能力边界**——写作前先确认能力"能看到什么/不能看到什么"，正文中禁止扩大或混淆能力表述（如把"查看行为日志"写成"读取系统内部"）。能力描述要以 craft 角色档案和 setup rules 为准，**不得臆造能力细节**
9.  **人设行为语境（铁律）**：主角/配角的行为习惯、职业背景、说话方式**必须取自 story_setup.core_cast 与 craft 角色档案
10. **记忆点克制（铁律）**：角色的标志性动作/口头禅（以 craft 档案记录为准，如"指节敲三下""日志不会说谎"）每章最多出现 **1 次**（关键节点可回扣，其余删除）
...
- `craft`: 可查询的世界元素详细档案（角色外貌/性格/动机/说话方式、地点氛围/历史、物品功能/来历）。**写作前建议用 `novelgen tool query context --type craft-character|craft-location|craft-item --name "<名称>" --view brief` 查询出场角色/场景/物品的详细档案**——大纲只有概要，细节档案在 craft 里，查询后角色描写、氛围、物品用法才准确
- `target_words`: 本章目标字数
...
6.  **减少状语（铁律）**：删掉无信息量的状语与程度副词——"轻轻地/缓缓地/慢慢地/静静地/深深地/默默地/微微地"这类"XX地"修饰，以及"十分/非常/格外/异常"等空泛程度词，能用动作本身传达就不用副词修饰；一段里"地"字状语不超过1-2处，情绪靠动作细节与画面传达，不靠状语堆砌
7.  **增强画面感（铁律）**：每个关键情绪点/动作点尽量落成"看得见的具体画面"——用具体的物象、动作、光线、声响替代抽象概括（如不写"气氛凝重"而写"烛火跳了跳，没人去扶"；不写"他很紧张"而写"他捻着杯沿，指节发白"）；读者应该"看见"场景而不是"被告知"情绪，画面优先于评价
...
## AI 味预防清单（生成时逐项自查，写出来之前先检查）
> 这是"去AI味"审查官的标准，**生成时就避免**，不是写完再改。以下每一项如果命中，说明这段文字"太像AI写的"：
1.  **金句密度**：对仗句、排比句、总结性警句（如"XX是命""散场时最贵"）每章最多 1-2 处，且必须自然出自人物处境，禁止章节每段结尾都塞金句、禁止"前世老师说过"这类万能金句触发器反复使用
2.  **模板化节奏**：禁止每章都完美走完"目标-冲突-解决-钩子"无意外、无粗糙感；允许章节有失败的尝试、无意义的闲笔、没解决的小事
...
8.  **元语言**：禁止正文出现"他心想，这是……"式的自我总结、禁止用"仿佛/宛如/像极了"过度修辞、禁止章节标题式总结感
