...
- Generation should use the current chapter, previous recap, nearby context,
  pre-chapter state matrix, and configured word target. The state matrix sent to
  write generation/review/improve represents facts before the target chapter
  begins; the target chapter's own events remain planned beats/deltas rather
  than already-true state.
